...
function check(name, condition, info) {
  if (condition) {
    ...
    console.log(`  ${colors.red}FAIL${colors.reset} ${name}${info ? `\n       ${colors.dim}${JSON.stringify(info)}${colors.reset}` : ''}`);
  }
}
...
async function run() {
  ...
  section('Health & readiness');
  {
    ...
    check('GET /health returns 200 healthy', health.status === 200 && health.body.status === 'healthy', health.body);

    const ready = await call('GET', '/ready');
    check('GET /ready reports mongodb and redis up',
      ready.status === 200 && ready.body.checks.mongodb && ready.body.checks.redis, ready.body);

    const missing = await call('GET', '/api/v1/does-not-exist');
    check('unknown route returns a structured 404',
      missing.status === 404 && missing.body.error.code === 'ROUTE_NOT_FOUND', missing.body);
  ...
  {
    const weak = await call('POST', '/api/v1/auth/register', { body: { email: `weak.${suffix}@example.com`, password: 'short' } });
    check('rejects a weak password', weak.status === 400, weak.body);

    const badEmail = await call('POST', '/api/v1/auth/register', { body: { email: 'not-an-email', password: PASSWORD } });
    check('rejects a malformed email', badEmail.status === 400, badEmail.body);

    const injection = await call('POST', '/api/v1/auth/register', { body: { email: { $ne: null }, password: PASSWORD } });
    check('rejects a NoSQL operator object as email', injection.status === 400, injection.body);

    const noSymbol = await call('POST', '/api/v1/auth/register', { body: { email: `nosym.${suffix}@example.com`, password: 'abcdefghij1A' } });
    ...
    check('registration never returns the password hash', !JSON.stringify(created.body).includes('passwordHash'));
    ...
    const dupCase = await call('POST', '/api/v1/auth/register', { body: { ...alice, email: alice.email.toUpperCase() } });
  ...
  {
    const wrong = await call('POST', '/api/v1/auth/login', { body: { email: alice.email, password: 'Wrong!Password1' } });
    check('rejects a wrong password with 401', wrong.status === 401, wrong.body);
    check('does not reveal whether the email exists', wrong.body?.error?.message === 'Invalid credentials', wrong.body);

    const unknown = await call('POST', '/api/v1/auth/login', { body: { email: `ghost.${suffix}@example.com`, password: PASSWORD } });
    check('unknown user returns the same error as a wrong password',
      unknown.status === 401 && unknown.body.error.message === 'Invalid credentials', unknown.body);

    const ok = await call('POST', '/api/v1/auth/login', { body: { email: alice.email, password: alice.password } });
    check('logs in and returns an access and refresh token',
      ok.status === 200 && ok.body.data.accessToken && ok.body.data.refreshToken, ok.body);
    alice.token = ok.body?.data?.accessToken;
    alice.refresh = ok.body?.data?.refreshToken;

    const bobLogin = await call('POST', '/api/v1/auth/login', { body: { email: bob.email, password: bob.password } });
    bob.token = bobLogin.body?.data?.accessToken;
    bob.refresh = bobLogin.body?.data?.refreshToken;

    await promoteToAdmin(admin.email);
    const adminLogin = await call('POST', '/api/v1/auth/login', { body: { email: admin.email, password: admin.password } });
    admin.token = adminLogin.body?.data?.accessToken;
    check('the promoted user logs in holding the ADMIN role',
      adminLogin.body?.data?.user?.roles?.includes('ADMIN'), adminLogin.body?.data?.user);
  }
  ...
  section('GET /api/v1/auth/me');
  {
    ...
    check('returns the caller profile', mine.status === 200 && mine.body.data.email === alice.email, mine.body);
  ...
  section('POST /api/v1/auth/refresh');
  {
    ...
    check('rotates the refresh token', rotated.status === 200 && rotated.body.data.refreshToken !== alice.refresh, rotated.body);
    ...
    const relogin = await call('POST', '/api/v1/auth/login', { body: { email: alice.email, password: alice.password } });
  ...
  section('POST /api/v1/accounts');
  {
    ...
    const badType = await call('POST', '/api/v1/accounts', { token: alice.token, body: { accountType: 'MATTRESS' } });
    check('rejects an unknown account type', badType.status === 400, badType.body);

    const badCurrency = await call('POST', '/api/v1/accounts', { token: alice.token, body: { accountType: 'SAVINGS', currency: 'XYZ' } });
    check('rejects an unsupported currency', badCurrency.status === 400, badCurrency.body);

    const created = await call('POST', '/api/v1/accounts', { token: alice.token, body: { accountType: 'SAVINGS', currency: 'INR' } });
    check('opens an account', created.status === 201 && created.body.data._id, created.body);
    check('the account number is server-generated', /^\d{16}$/.test(created.body?.data?.accountNumber || ''), created.body?.data);
    alice.account = created.body?.data?._id;

    const minted = await call('POST', '/api/v1/accounts', {
      token: alice.token,
      body: { accountType: 'WALLET', currency: 'INR', balance: 999999999, availableBalance: 999999999, status: 'ACTIVE' },
    });
    check('a client-supplied opening balance is ignored (no money minting)',
      minted.status === 201 && minted.body.data.balance === 0 && minted.body.data.availableBalance === 0, minted.body?.data);
    alice.wallet = minted.body?.data?._id;

    const forOther = await call('POST', '/api/v1/accounts', { token: alice.token, body: { accountType: 'SAVINGS', userId: bob.id } });
    check('a non-admin cannot open an account for someone else', forOther.status === 403, forOther.body);

    const usd = await call('POST', '/api/v1/accounts', { token: alice.token, body: { accountType: 'CURRENT', currency: 'USD' } });
    alice.usdAccount = usd.body?.data?._id;

    const bobAccount = await call('POST', '/api/v1/accounts', { token: bob.token, body: { accountType: 'SAVINGS', currency: 'INR' } });
    bob.account = bobAccount.body?.data?._id;
    check('the second user also gets an account', bobAccount.status === 201, bobAccount.body);

    const adminAccount = await call('POST', '/api/v1/accounts', { token: admin.token, body: { accountType: 'CURRENT', currency: 'INR' } });
  ...
  section('GET /api/v1/accounts');
  {
    ...
    check('lists only the caller accounts',
      mine.status === 200 && mine.body.data.every((a) => a.userId === alice.id), mine.body?.data?.map((a) => a.userId));
  ...
  section('PATCH /api/v1/accounts/:id');
  {
    ...
    check('updates metadata', meta.status === 200 && meta.body.data.metadata.nickname === 'Rainy day', meta.body);

    const tamper = await call('PATCH', `/api/v1/accounts/${alice.account}`, {
      token: alice.token,
      body: { balance: 500000, availableBalance: 500000, status: 'ACTIVE' },
    });
    check('a balance sent to PATCH is ignored (mass assignment blocked)',
      tamper.status === 200 && tamper.body.data.balance === 0, tamper.body?.data);

    const theirs = await call('PATCH', `/api/v1/accounts/${bob.account}`, { token: alice.token, body: { metadata: {} } });
  ...
  {
    const anon = await call('POST', '/api/v1/transactions/deposit', { body: { accountId: alice.account, amount: 100 } });
    check('requires authentication', anon.status === 401, anon.body);

    for (const [label, amount] of [['zero', 0], ['negative', -5000], ['fractional', 10.5], ['string', '1000'], ['NaN', 'abc'], ['huge', 1e18]]) {
      const res = await call('POST', '/api/v1/transactions/deposit', { token: alice.token, body: { accountId: alice.account, amount } });
    ...
    check('deposits 500.00 INR', ok.status === 201 && ok.body.data.amount === 50000 && ok.body.data.status === 'COMPLETED', ok.body);
    ...
    check('the cached balance matches the ledger', balance.body?.data?.reconciled === true, balance.body?.data);

    const theirs = await call('POST', '/api/v1/transactions/deposit', { token: alice.token, body: { accountId: bob.account, amount: 10000 } });
    check("cannot deposit into another user's account", theirs.status === 404, theirs.body);

    // Fund the others for the transfer tests.
    await call('POST', '/api/v1/transactions/deposit', { token: bob.token, body: { accountId: bob.account, amount: 200_00 } });
    await call('POST', '/api/v1/transactions/deposit', { token: alice.token, body: { accountId: alice.usdAccount, amount: 100_00 } });
  }
  ...
  section('POST /api/v1/transactions (transfer)');
  {
    ...
    check('rejects an overdraft', overdraft.status === 400 && overdraft.body.error.code === 'INSUFFICIENT_FUNDS', overdraft.body);
    ...
    check('both sides stay reconciled with the ledger',
  ...
  section('Idempotency');
  {
    ...
    const body = { fromAccount: alice.account, toAccount: bob.account, amount: 25_00, reference: 'Idempotent' };

    const first = await call('POST', '/api/v1/transactions', { token: alice.token, body, headers: { 'X-Idempotency-Key': key } });
    check('the first request succeeds', first.status === 201, first.body);

    const replay = await call('POST', '/api/v1/transactions', { token: alice.token, body, headers: { 'X-Idempotency-Key': key } });
    check('a replay returns the same transaction', replay.body?.data?._id === first.body?.data?._id, { first: first.body?.data?._id, replay: replay.body?.data?._id });
    ...
    check('the same key with a different payload is rejected', differentBody.status === 409, differentBody.body);
    ...
    check("another user's identical key is not a cache hit (keys are per-user)",
      bobReuse.status === 201 && bobReuse.body?.data?._id !== first.body?.data?._id, bobReuse.body);

    const badKey = await call('POST', '/api/v1/transactions', {
      token: alice.token, body, headers: { 'X-Idempotency-Key': 'has spaces and #' },
    });
    ...
    const before = (await call('GET', `/api/v1/accounts/${alice.account}/balance`, { token: alice.token })).body.data.balance;
    const results = await Promise.all(
      Array.from({ length: 10 }, () =>
        call('POST', '/api/v1/transactions', { token: alice.token, body: raceBody, headers: { 'X-Idempotency-Key': raceKey } })
      )
    );
    const after = (await call('GET', `/api/v1/accounts/${alice.account}/balance`, { token: alice.token })).body.data.balance;
    const created = results.filter((r) => r.status === 201).length;
    check('10 concurrent identical requests debit the account once',
      before - after === 1000, { before, after, statuses: results.map((r) => r.status) });
    check('concurrent duplicates are either replayed or refused, never double-spent',
      created >= 1 && results.every((r) => [201, 409].includes(r.status)), results.map((r) => r.status));
  }
  ...
  section('Concurrency without an idempotency key');
  {
    ...
    const balanceBefore = (await call('GET', `/api/v1/accounts/${alice.account}/balance`, { token: alice.token })).body.data.balance;
    const chunk = Math.floor(balanceBefore / 2);
    const results = await Promise.all(
      Array.from({ length: 5 }, () =>
        call('POST', '/api/v1/transactions', { token: alice.token, body: { fromAccount: alice.account, toAccount: bob.account, amount: chunk } })
      )
    );
    const ok = results.filter((r) => r.status === 201).length;
    const balanceAfter = (await call('GET', `/api/v1/accounts/${alice.account}/balance`, { token: alice.token })).body.data.balance;

    check('concurrent spends cannot overdraw the account', balanceAfter >= 0, { balanceBefore, balanceAfter, ok });
    check('the debited total matches the transfers that succeeded',
      balanceBefore - balanceAfter === ok * chunk, { balanceBefore, balanceAfter, ok, chunk });
  }

  section('POST /api/v1/transactions/withdraw');
  {
    const tooMuch = await call('POST', '/api/v1/transactions/withdraw', { token: bob.token, body: { accountId: bob.account, amount: 999_999_00 } });
    check('rejects a withdrawal above the balance', tooMuch.status === 400, tooMuch.body);

    const before = (await call('GET', `/api/v1/accounts/${bob.account}/balance`, { token: bob.token })).body.data.balance;
    const ok = await call('POST', '/api/v1/transactions/withdraw', { token: bob.token, body: { accountId: bob.account, amount: 50_00 } });
    check('withdraws 50.00 INR', ok.status === 201 && ok.body.data.type === 'WITHDRAWAL', ok.body);

    const after = (await call('GET', `/api/v1/accounts/${bob.account}/balance`, { token: bob.token })).body.data.balance;
    check('the withdrawal reduced the balance by exactly the amount', before - after === 5000, { before, after });
  }

  section('Frozen and closed accounts');
  {
    const freeze = await call('POST', `/api/v1/accounts/${alice.wallet}/freeze`, { token: alice.token });
    check('an owner can freeze their account', freeze.status === 200 && freeze.body.data.status === 'FROZEN', freeze.body);

    const intoFrozen = await call('POST', '/api/v1/transactions', {
      token: bob.token, body: { fromAccount: bob.account, toAccount: alice.wallet, amount: 100 },
    });
    check('a frozen account cannot receive money', intoFrozen.status === 409, intoFrozen.body);

    const depositFrozen = await call('POST', '/api/v1/transactions/deposit', { token: alice.token, body: { accountId: alice.wallet, amount: 100 } });
    check('a frozen account cannot be deposited into', depositFrozen.status === 409, depositFrozen.body);

    const selfUnfreeze = await call('POST', `/api/v1/accounts/${alice.wallet}/unfreeze`, { token: alice.token });
    check('a user cannot unfreeze their own account', selfUnfreeze.status === 403, selfUnfreeze.body);

    const adminUnfreeze = await call('POST', `/api/v1/accounts/${alice.wallet}/unfreeze`, { token: admin.token });
    check('an admin can unfreeze it', adminUnfreeze.status === 200 && adminUnfreeze.body.data.status === 'ACTIVE', adminUnfreeze.body);
    ...
    check('an empty account can be closed', closeEmpty.status === 200 && closeEmpty.body.data.status === 'CLOSED', closeEmpty.body);

    const useClosed = await call('POST', '/api/v1/transactions/deposit', { token: alice.token, body: { accountId: alice.wallet, amount: 100 } });
  ...
  section('GET /api/v1/transactions');
  {
    ...
    const stranger = await call('GET', `/api/v1/transactions/${alice.transferId}`, { token: admin.token ? undefined : undefined });
    check('an anonymous caller cannot', stranger.status === 401, stranger.body);

    const list = await call('GET', `/api/v1/transactions/account/${alice.account}?limit=5`, { token: alice.token });
    check('lists transactions for an owned account', list.status === 200 && Array.isArray(list.body.data), list.body);
  ...
  {
    const asUser = await call('POST', `/api/v1/transactions/${alice.transferId}/reverse`, { token: alice.token, body: { reason: 'oops' } });
    check('a normal user cannot reverse a transaction', asUser.status === 403, asUser.body);

    const bobBefore = (await call('GET', `/api/v1/accounts/${bob.account}/balance`, { token: bob.token })).body.data.balance;
    const reversed = await call('POST', `/api/v1/transactions/${alice.transferId}/reverse`, { token: admin.token, body: { reason: 'Disputed' } });
    check('an admin can reverse it', reversed.status === 201, reversed.body);

    const bobAfter = (await call('GET', `/api/v1/accounts/${bob.account}/balance`, { token: bob.token })).body.data.balance;
    ...
    check('the original is marked REVERSED, not deleted', original.body?.data?.status === 'REVERSED', original.body?.data);

    const again = await call('POST', `/api/v1/transactions/${alice.transferId}/reverse`, { token: admin.token });
  ...
  section('GET /api/v1/ledger');
  {
    ...
    check('returns journal entries for an owned account', mine.status === 200 && mine.body.data.length > 0, mine.body?.pagination);
    check('entries record the balance after each movement',
      mine.body?.data?.every((e) => typeof e.balanceAfter === 'number'), mine.body?.data?.[0]);
    ...
    const byTxn = await call('GET', `/api/v1/ledger/transactions/${alice.transferId}`, { token: alice.token });
    check('returns both legs of a transaction', byTxn.status === 200 && byTxn.body.data.length === 2, byTxn.body?.data?.length);
    const legs = byTxn.body?.data ?? [];
    check('the two legs are a matching debit and credit',
      legs.length === 2 && legs[0].amount === legs[1].amount && new Set(legs.map((l) => l.entryType)).size === 2, legs);

    const reconcile = await call('GET', `/api/v1/ledger/accounts/${alice.account}/reconcile`, { token: alice.token });
    check('per-account reconciliation passes', reconcile.body?.data?.reconciled === true, reconcile.body?.data);

    const verifyAsUser = await call('GET', '/api/v1/ledger/verify', { token: alice.token });
    check('only an admin can run the system-wide verification', verifyAsUser.status === 403, verifyAsUser.body);

    const verify = await call('GET', '/api/v1/ledger/verify', { token: admin.token });
    check('total debits equal total credits across the whole ledger',
      verify.status === 200 && verify.body.data.balanced === true, verify.body?.data);
  }

  section('POST /api/v1/auth/logout & change-password');
  {
    const logout = await call('POST', '/api/v1/auth/logout', { token: bob.token, body: { refreshToken: bob.refresh } });
    ...
    const refreshAfterLogout = await call('POST', '/api/v1/auth/refresh', { body: { refreshToken: bob.refresh } });
    check('the refresh token is revoked too', refreshAfterLogout.status === 401, refreshAfterLogout.body);

    const relogin = await call('POST', '/api/v1/auth/login', { body: { email: bob.email, password: bob.password } });
    bob.token = relogin.body?.data?.accessToken;

    const weak = await call('POST', '/api/v1/auth/change-password', { token: bob.token, body: { currentPassword: bob.password, newPassword: 'weak' } });
    check('rejects a weak new password', weak.status === 400, weak.body);

    const wrongCurrent = await call('POST', '/api/v1/auth/change-password', { token: bob.token, body: { currentPassword: 'Nope!Nope!1', newPassword: 'An0ther!Strong1' } });
    check('rejects a wrong current password', wrongCurrent.status === 401, wrongCurrent.body);

    const changed = await call('POST', '/api/v1/auth/change-password', { token: bob.token, body: { currentPassword: bob.password, newPassword: 'An0ther!Strong1' } });
    ...
    const oldPassword = await call('POST', '/api/v1/auth/login', { body: { email: bob.email, password: bob.password } });
    check('the old password no longer works', oldPassword.status === 401, oldPassword.body);

    const newPassword = await call('POST', '/api/v1/auth/login', { body: { email: bob.email, password: 'An0ther!Strong1' } });
  ...
  section('Hardening');
  {
    ...
    check('an oversized body is rejected', oversized.status === 413 || oversized.status === 400, oversized.status);

    const injection = await call('POST', '/api/v1/transactions', {
      token: alice.token, body: { fromAccount: { $ne: null }, toAccount: bob.account, amount: 100 },
    });
    ...
    check('errors share one shape with a code and a requestId',
      errorShape.body?.success === false && errorShape.body?.error?.code && errorShape.body?.error?.requestId, errorShape.body);
  }
